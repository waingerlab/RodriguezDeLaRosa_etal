...
function graph = beeswarmbar_changecolors(datacolumns,proximity,lim,points,pointlabels,xspread,pointsize)
...
for n=1:groupnum
    ...
    %changed
    if n ==2
    scatter(position,sort(data{n}),pointsize,[.2 .2 .2],'filled','b');
    else
    ...
    if n == 2 %this first if statement can be used to change the color of a specific bar
    color = 'c';
